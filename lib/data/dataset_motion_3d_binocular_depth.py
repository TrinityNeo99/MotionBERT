...
class MotionDataset3D(MotionDataset):
    ...
    def __getitem__(self, index):
        ...
        motion_depth = motion_file['data_depth']
        if self.data_split == "train":
            if self.synthetic or self.gt_2d:
                motion_3d = self.aug.augment3D(motion_3d)
                motion_2d_left = np.zeros(motion_3d.shape, dtype=np.float32)
                motion_2d_left[:, :, :2] = motion_3d[:, :, :2]
                motion_2d_left[:, :, 2] = 1  # No 2D detection, use GT xy and c=1.
                motion_2d_right = motion_2d_left
            elif motion_file["data_left"] is not None:  # Have 2D detection
                motion_2d_left = motion_file["data_left"]
                motion_2d_right = motion_file["data_right"]

                if self.flip and random.random() > 0.5:  # Training augmentation - random flipping
                    motion_2d_left = flip_data(motion_2d_left)
                    motion_2d_right = flip_data(motion_2d_right)
        ...
        return torch.FloatTensor(motion_2d_left), torch.FloatTensor(motion_2d_right), torch.FloatTensor(motion_3d), torch.FloatTensor(motion_depth)
