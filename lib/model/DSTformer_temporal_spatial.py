...
from lib.model.MoE_temporal_module import MoE_temporal_module as MoE_Trans
from einops import rearrange
from lib.graph.pingpong_coco import AdjMatrixGraph as Graph
...
class Block(nn.Module):
    ...
    def forward(self, x, seqlen=1):
        if self.st_mode == 'stage_st':
            ...
            x = rearrange(x, "(B F) J C -> B C F J", F=seqlen)
            x = self.temporal_trans(x)
...
class DSTformer_temporal_spatial(nn.Module):
